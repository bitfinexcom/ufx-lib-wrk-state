...
class CoreSym extends CoreBase {
  ...
  updateEntryFx (type, entry) {
    ...
    if (entry.symbol_real === entry.symbol) {
      fx = '1'
    } else {
      entry._fx_req = 1
  ...
  handleTrade (type, msg) {
    const trade = this.lib_spec.trade(msg)
    ...
    this.qout_0.push([
      ...
      this.symMsg(['trade', trade])
    ])

    if (trade.symbol[0] === 'f') {
      this.sendTradePriv('core_trade', trade)
      this.handleTradeRcn('trade_rcn', msg)
    }
  }

  handleTradeRcn (type, msg) {
    const trade = this.lib_spec.trade_rcn(msg)
    trade._rec = 1
    ...
    this.sendTradePriv('core_trade_rcn', trade)
  }
  ...
  sendTradePriv (action, trade) {
    _.each(['b', 's'], pfx => {
      ...
      this.qout_0.push([
        'core.sym', 'publish',
        `${this.conf.gw_ipfx}.te_priv.r${this.getRingId(ctrade.user_id, 'priv')}`,
  ...
  subSymShared () {
    ...
    const ccys_fiat = conf.ccys_fiat || []
