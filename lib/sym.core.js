...
class CoreSym extends CoreBase {
  ...
  refreshEntriesFx (type, opts = {}) {
    const maxDepth = opts.book_depth || 0.5
  ...
  handleEntry (type, msg) {
    ...
    const entry = this.lib_spec[type](msg, _entry)
    if (!entry) {
      return
    }
  ...
  sendEntryChange (entry, isActive) {
    ...
    if (entry._book) {
      this.qout_0.push([
        ...
        ['bu', this.prepEntryRaw(st, centry, isActive)]
      ])
    }
  }
  ...
  oJobHandle (job) {
    ...
    switch (job[0]) {
      case 'core.sym':
        part = {}
        part[`gw${this.busIx}`] = [[job[1], job[2], JSON.stringify(job[3])]]
        break
      case 'core.sym-book':
        job[3].push(this.mem.symBookSix, this.mem.symBookSeq++)
        part = {
          bm: [[job[1], job[2], JSON.stringify(job[3])]]
        }
        break
      case 'core.sym-stream':
        job[3].push(this.mem.symStreamSix, this.mem.symStreamSeq++)
        part = {
          bm: [[job[1], job[2], JSON.stringify(job[3])]]
        }
        break
