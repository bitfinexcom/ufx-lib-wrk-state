...
const specLib = require('../lib/spec.parse')
...
class CorePriv extends CoreBase {
  ...
  handleRedisStream (channel, msg, src) {
    ...
    if (chp[2] === 'global') {
      ...
      this.handlePriv({ a: 'user_snap', o: msg })
    } else {
      this.handlePriv(msg)
    }
  }
  ...
  handleUser (type, msg, opts = {}) {
    ...
    switch (type) {
      case 'conf':
        ...
        subject = [['conf', user.conf, {
          useSeq: true
        }]]
        break
      case 'auth_token':
        subject = [['auth_token', msg, {}]]
        break
      case 'notify':
        subject = [['notify', msg, {
    ...
    if (subject) {
      _.each(subject, subject => {
        const edata = subject[1]

        if (msg.s) {
          edata.s = msg.s
        }

        pubData.push(subject)
      })
    }

    this.qout_0.push([
      'core.priv',
      { a: 'pub_user', user_id: user.id, data: pubData }
    ])
  }
  ...
  sendUserSnap (uid, opts) {
    ...
    const udata = snapType === 'live' ? _.pick(user, this.conf.liveFields) : user

    this.qout_0.push(
      [
        'core.priv', { a: 'pub_snap', user_id: user.id, data: _.cloneDeep(udata) },
  ...
  handleEntryBase (user, type, msg, opts) {
    ...
    const entry = parser(msg, live[eid])
    ...
    subject.push([type, _.clone(entry), {
      useSeq: true
    }])
  ...
  handlePosition (user, type, msg, opts) {
    ...
    const pos = this.lib_spec.pos(msg, _pos)
    ...
    return [['position', _.clone(pos), {
      useSeq: true
    }]]
  }

  getPubSnap (user, data) {
    const msg = JSON.stringify([user.id, 'user', data])
  ...
  getPubUser (user, entries) {
    ...
    _.each(entries, entry => {
      ...
      const msg = JSON.stringify([user.id, type, payload, user.seq])
