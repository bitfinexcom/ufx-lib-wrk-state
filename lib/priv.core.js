...
class CorePriv extends CoreBase {
  ...
  getPubSnap (user, data) {
    ...
    ret[`gw${this.busIx}`] = [
      ...
      ['publish', `${this.conf.gw_ipfx}.priv.hw.${user.id}`, data]
    ]
  ...
  getPubUser (user, entry) {
    ...
    bus.push(
      ...
      ['publish', `${this.conf.gw_ipfx}.priv.hw.${user.id}`, msg]
    )

    if (opts.useSpecChannel) {
      bus.push(
        ['publish', `${this.conf.gw_ipfx}.priv_${type}.${this.ring}`, msg],
        ['publish', `${this.conf.gw_ipfx}.priv_${type}.hw.${user.id}`, msg],
      )
    }
